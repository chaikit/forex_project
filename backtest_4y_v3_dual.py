...
# Custom Backtester for Dual Setup (4-Year Period)
# Simulates both Fibo 0.618 and 0.786 entries on the same day if hit.
class BacktesterV3_4YearsDual:
    def __init__(self, symbol="EURUSD", initial_balance=1000.0, risk_percent=1.0):
        ...
        # Exact settings from trade_setup_v3.py
        self.sl_points = 15
        self.tp_points = 60
        self.rr_ratio = 4.0
    ...
    def run(self, df, allowed_weekdays=None):
        ...
        for date in days:
            ...
            # Strategy looks for range between 9:00 and 12:00
            ...
            # Risk calculation at the start of the session (similar to live trade logic)
            # Note: In backtest, balance changes as trades close.
            
            if low_time < high_time: # BUY SETUP
                for fibo in self.fibo_levels:
                    entry = round(high_price - (price_range * fibo), 5)
                    # NO BREAK here - allow both fibos to be simulated
            ...
            if (side == "BUY" and row.low <= entry) or (side == "SELL" and row.high >= entry):
                ...
                for t_row in df.iloc[search_idx_start:].itertuples():
                    if t_row.time >= force_close:
                        ...
                        self.record_trade(entry_time, t_row.time, side, f"v3.0 Dual Fibo {fibo}", entry, t_row.open, pf, "Force Close")
                        return True
                    
                    if (side == "BUY" and t_row.low <= sl) or (side == "SELL" and t_row.high >= sl):
                        self.record_trade(entry_time, t_row.time, side, f"v3.0 Dual Fibo {fibo}", entry, sl, -1.0, "SL")
                        return True
                        
                    if (side == "BUY" and t_row.high >= tp) or (side == "SELL" and t_row.low <= tp):
                        self.record_trade(entry_time, t_row.time, side, f"v3.0 Dual Fibo {fibo}", entry, tp, self.rr_ratio, "TP")
                        return True
                return True
        return False
...
if __name__ == "__main__":
    tester = BacktesterV3_4YearsDual()
    ...
    if data is not None:
        ...
        suffix = "4y_v3_Dual_Setup"
        if not os.path.exists("reports"): os.makedirs("reports")
        
        logger = LogProcessor(tester.trades, start_hour=12, close_hour=0, tp_multiplier=4.0)
