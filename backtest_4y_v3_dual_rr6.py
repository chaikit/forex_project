...
# Custom Backtester for Dual Setup with RR 1:6 (4-Year Period)
class BacktesterV3_4YearsDualRR6:
    def __init__(self, symbol="EURUSD", initial_balance=1000.0, risk_percent=1.0):
        ...
        # Scaling settings for RR 1:6
        self.sl_points = 15
        self.tp_points = 90 # 15 * 6
        self.rr_ratio = 6.0
            ...
            if (side == "BUY" and row.low <= entry) or (side == "SELL" and row.high >= entry):
                ...
                for t_row in df.iloc[search_idx_start:].itertuples():
                    if t_row.time >= force_close:
                        ...
                        self.record_trade(entry_time, t_row.time, side, f"v3.0 Dual Fibo {fibo} RR 1:6", entry, t_row.open, pf, "Force Close")
                        return True
                    if (side == "BUY" and t_row.low <= sl) or (side == "SELL" and t_row.high >= sl):
                        self.record_trade(entry_time, t_row.time, side, f"v3.0 Dual Fibo {fibo} RR 1:6", entry, sl, -1.0, "SL")
                        return True
                    if (side == "BUY" and t_row.high >= tp) or (side == "SELL" and t_row.low <= tp):
                        self.record_trade(entry_time, t_row.time, side, f"v3.0 Dual Fibo {fibo} RR 1:6", entry, tp, self.rr_ratio, "TP")
                        return True
                return True
        return False
...
if __name__ == "__main__":
    tester = BacktesterV3_4YearsDualRR6()
