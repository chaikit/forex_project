...
# Custom Backtester for Dual Setup with Hour Filter (4-Year Period)
# Excludes entries triggered during 18:00 - 18:59 to improve Win Rate
class BacktesterV3_4YearsFiltered:
    def __init__(self, symbol="EURUSD", initial_balance=1000.0, risk_percent=1.0):
        ...
        self.start_hour = 12
        self.close_hour = 0
        self.exclude_hours = [18] # Exclude 18:00 entries
    ...
    def simulate_trade(self, df, start_idx, side, entry, sl, tp, force_close, fibo):
        ...
        for idx_rel, row in enumerate(future_df.itertuples()):
            ...
            if (side == "BUY" and row.low <= entry) or (side == "SELL" and row.high >= entry):
                ...
                # HOUR FILTER CHECK
                if entry_time.hour in self.exclude_hours:
                    return False # Skip this trade if triggered during excluded hour
                ...
                for t_row in df.iloc[search_idx_start:].itertuples():
                    if t_row.time >= force_close:
                        ...
                        self.record_trade(entry_time, t_row.time, side, f"v3.0 Filtered RR1:6", entry, t_row.open, pf, "Force Close")
                        return True
                    if (side == "BUY" and t_row.low <= sl) or (side == "SELL" and t_row.high >= sl):
                        self.record_trade(entry_time, t_row.time, side, f"v3.0 Filtered RR1:6", entry, sl, -1.0, "SL")
                        return True
                    if (side == "BUY" and t_row.high >= tp) or (side == "SELL" and t_row.low <= tp):
                        self.record_trade(entry_time, t_row.time, side, f"v3.0 Filtered RR1:6", entry, tp, self.rr_ratio, "TP")
                        return True
                return True
        return False
...
if __name__ == "__main__":
    tester = BacktesterV3_4YearsFiltered()
