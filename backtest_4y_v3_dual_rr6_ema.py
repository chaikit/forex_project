...
# Custom Backtester with EMA 200 Trend Filter (4-Year Period)
# Buy only if Price > EMA 200 | Sell only if Price < EMA 200
class BacktesterV3_4YearsEMA:
    ...
    def get_data(self, start_date):
        ...
        # Fetch extra data (around 200 bars before start) to calculate EMA correctly from the beginning
    ...
    def simulate_trade(self, df, start_idx, side, entry, sl, tp, force_close, fibo):
        ...
        # Get the actual position in the full dataframe to access EMA
        for idx_rel, row in enumerate(future_df.itertuples()):
            ...
            if (side == "BUY" and row.low <= entry) or (side == "SELL" and row.high >= entry):
                ...
                # EMA FILTER Logic: Buy > EMA, Sell < EMA
                ...
                for t_row in df.iloc[search_idx_start:].itertuples():
                    if t_row.time >= force_close:
                        ...
                        self.record_trade(entry_time, t_row.time, side, f"v3.0 EMA Filtered RR1:6", entry, t_row.open, pf, "Force Close")
                        return True
                    
                    if (side == "BUY" and t_row.low <= sl) or (side == "SELL" and t_row.high >= sl):
                        self.record_trade(entry_time, t_row.time, side, f"v3.0 EMA Filtered RR1:6", entry, sl, -1.0, "SL")
                        return True
                        
                    if (side == "BUY" and t_row.high >= tp) or (side == "SELL" and t_row.low <= tp):
                        self.record_trade(entry_time, t_row.time, side, f"v3.0 EMA Filtered RR1:6", entry, tp, self.rr_ratio, "TP")
                        return True
                return True
        return False
